...
- 시뮬 코어 src/sim.py 로 분리(단계별 R32~결승 도달 확률 집계).
- 깃 레포 2개(public): 모델 github.com/choigod1023/wc2026-predictor,
  웹 github.com/choigod1023/wc2026-web (Next.js, Vercel 배포).
  웹 탭: 대시보드/라이브/모델비교/토너먼트/수식. 라이브는 named.com API(키 불필요)를
  Next 서버 라우트(app/api/live)로 프록시(CORS 회피)해 스코어·경기별 3-way 배당·실시간
  조별 순위 제공. 팀명 매핑 lib/teams.ts, 조 복원/순위 lib/groups.ts.
